...
class CapLSMT_c3(TextModel):
    def get_model(self, trainable=None):
        ...
        x = Bidirectional(CuDNNLSTM(cfg.LSTM_hidden_size, return_sequences=True))(emb)
        x = Bidirectional(CuDNNLSTM(cfg.LSTM_hidden_size, return_sequences=True))(x)
        x = Capsule(10, 32, routings=5, share_weights=True)(x)
        x = Flatten()(x)
        x = Dropout(0.5)(x)
        x = Dense(5, activation='relu')(x)
        x = Dense(1, activation='linear')(x)
...
class CapLSMT_c4(TextModel):
    def get_model(self, trainable=None):
        ...
        x = Bidirectional(CuDNNLSTM(cfg.LSTM_hidden_size, return_sequences=True))(emb)
        x = Bidirectional(CuDNNLSTM(cfg.LSTM_hidden_size, return_sequences=True))(x)
        x = Capsule(20, 16, routings=5, share_weights=True)(x)
        x = Flatten()(x)
        x = Dropout(0.5)(x)
        x = Dense(5, activation='relu')(x)
        x = Dense(1, activation='linear')(x)
